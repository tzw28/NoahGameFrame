 ...
 */
    int ret = evhttp_bind_socket(http, "0.0.0.0", port);
    if (ret != 0)
    {
        std::cout << "bind port :" << port << " fail" << std::endl;;
        return 1;
...
bool NFHttpServer::ResponseMsg(NF_SHARE_PTR<NFHttpRequest> req, const std::string& msg, NFWebStatus code, const std::string& strReason)
{
    ...
    //send data
    evbuffer_add(eventBuffer, msg.c_str(), msg.size());
    // evbuffer_add_printf(eventBuffer, "%.*s", msg.size(), msg.c_str());

    evhttp_add_header(evhttp_request_get_output_headers(pHttpReq), "Content-Type", "application/json");
    // gzip flag
    evhttp_add_header(evhttp_request_get_output_headers(pHttpReq), "Content-Encoding", "gzip");
